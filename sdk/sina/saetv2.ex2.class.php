 ...
 * @ignore
 */
class OAuthException3 extends Exception {
	// pass
}
...
class SaeTOAuthV3 {
	...
	function getAccessToken( $type = 'code', $keys ) {
		...
		if ( $type === 'token' ) {
			...
			throw new OAuthException3("wrong auth type");
		}
		...
		if ( is_array($token) && !isset($token['error']) ) {
			...
			throw new OAuthException3("get access token failed." . $token['error']);
		}
		return $token;
	}
...
class SaeTClientV3
{
	...
	{
		$this->oauth = new SaeTOAuthV3( $akey, $skey, $access_token, $refresh_token );
	}

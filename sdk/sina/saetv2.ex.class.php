...
class SaeTOAuthV2 {
	 ...
	 * @ignore
	 */
	public $timeout = 5;//iammutex modify
	/**
	 * Set connect timeout.
	 *
	 * @ignore
	 */
	public $connecttimeout = 5;//iammutex modify
	/**
	 * Verify SSL Cert.
	 *
	 * @ignore
...
//		$params['forcelogin'] = 'true';//iammutex add this
	...
	function getAccessToken( $type = 'code', $keys ) {
		...
		if ( $type === 'token' ) {
			...
			throw new OAuthException2("wrong auth type");
		}
		...
		if ( is_array($token) && !isset($token['error']) ) {
			...
			throw new OAuthException2("get access token failed." . $token['error']);
		}
		return $token;
	}
	...
	function getTokenInfo( $accesstoken ) {
		...
		if ( $accesstoken ) {
			...
			throw new OAuthException2("wrong access_token");
		}

		$response = $this->oAuthRequest($this->getTokenInfoURL(), 'POST', $params);
		$token = json_decode($response, true);
		if ( !is_array($token) || isset($token['error']) ) {
			throw new OAuthException2("get token info failed." . $token['error']);
		}
		return $token;
	}
